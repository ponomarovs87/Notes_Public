...
## 📖 Оглавление
- [KDE connect в OpenVPN](#kde-connect-в-openvpn)
  - [📖 Оглавление](#-оглавление)
- [Проблема](#проблема)
- [Цель](#цель)
- [Краткое решение](#краткое-решение)
- [1 Короткая идея](#1-короткая-идея)
- [2 Перезапускаем OpenVPN](#2-перезапускаем-openvpn)
  - [Пример фрагмента server.conf (на сервере)](#пример-фрагмента-serverconf-на-сервере)
    - [В server.conf (или /etc/openvpn/server/server.conf)](#в-serverconf-или-etcopenvpnserverserverconf)
    - [используем "subnet" — проще для статических адресов](#используем-subnet--проще-для-статических-адресов)
    - [файлы пулы и persist](#файлы-пулы-и-persist)
    - [разрешаем клиентам общаться друг с другом (ВАЖНО для KDE Connect)](#разрешаем-клиентам-общаться-друг-с-другом-важно-для-kde-connect)
    - [каталог с индивидуальными конфигурациями клиентов](#каталог-с-индивидуальными-конфигурациями-клиентов)
    - [уровень логов (по желанию)](#уровень-логов-по-желанию)
- [3 Создаём каталог CCD](#3-создаём-каталог-ccd)
- [4 Как назвать файл в ccd и что туда писать](#4-как-назвать-файл-в-ccd-и-что-туда-писать)
- [5 Важно: клиент должен иметь уникальный сертификат (CN)](#5-важно-клиент-должен-иметь-уникальный-сертификат-cn)
- [6 Дополнительно: маршрутизация и firewall](#6-дополнительно-маршрутизация-и-firewall)
- [7 Перезапуск OpenVPN](#7-перезапуск-openvpn)
      - [или](#или)
- [8 Проверьте логи:](#8-проверьте-логи)
- [9 Как узнать CN клиента (если не помните)](#9-как-узнать-cn-клиента-если-не-помните)
- [10 Что делать, если KDE Connect не видит устройство](#10-что-делать-если-kde-connect-не-видит-устройство)
- [11 Пример полной мини‑конфигурации](#11-пример-полной-миниконфигурации)
- [12 Пул ограничить кол-во выдаваемых ip (необязательно)](#12-пул-ограничить-кол-во-выдаваемых-ip-необязательно)
  - [Что решает:](#что-решает)
    - [Простое решение :](#простое-решение-)
    - [Непростое решение :](#непростое-решение-)
- [P.S. Примеры полные условно :](#ps-примеры-полные-условно-)
  - [а. Скопируй этот вариант и замени им существующий **/etc/openvpn/server/server.conf** (или /etc/openvpn/server.conf — если у тебя именно он используется):](#а-скопируй-этот-вариант-и-замени-им-существующий-etcopenvpnserverserverconf-или-etcopenvpnserverconf--если-у-тебя-именно-он-используется)
  - [б. Что теперь:](#б-что-теперь)
  - [в. Создай файлы клиентов:](#в-создай-файлы-клиентов)
  - [г. Перезапусти сервер:](#г-перезапусти-сервер)
...
*порт 1194 практически всегда заблокирован использовать не советую переделать на другой разблокированный в идеале использовать имитацию http/https tсl соединение*

### используем "subnet" — проще для статических адресов
...
чтобы сохранить в **/etc/sysctl.conf: net.ipv4.ip_forward=1**:
...
```
* или **/var/log/syslog** **/var/log/openvpn.log** в зависимости от системы
...
Если пинга нет — проверьте client-to-client, ip_forward и правила фаервола.
...
* выдайте фиксированным адреса с конца (если у вас менее 255 устройств одновременно все ок)
...
# Пользователь и группа для безопасности
user nobody
group nogroup
persist-key
persist-tun
...
# Сохраняем пул IP-адресов (необязательно, но полезно)
ifconfig-pool-persist /etc/openvpn/ipp.txt

# Разрешаем клиентам видеть друг друга (для KDE Connect и т.п.)
client-to-client

# Каталог с индивидуальными конфигами клиентов (статические IP)
client-config-dir /etc/openvpn/ccd

############################################################
# Маршрутизация и DNS
############################################################
# Принудительно направляем весь трафик клиентов через VPN
...
# (при желании можно добавить свои маршруты)
...
# Оптимизация TCP
############################################################
# Ограничение MSS для стабильной работы TCP-over-TCP
mssfix 1200
